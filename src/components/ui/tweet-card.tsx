...
function entityKey(
  entity: EnrichedTweet["entities"][number],
  index: number,
): string {
  return `${entity.type}-${entity.text}-${"href" in entity ? entity.href : ""}-${index}`;
}
        ...
        case "text":
          return (
            <span
              ...
              // biome-ignore lint/security/noDangerouslySetInnerHtml: tweet text is sanitized by react-tweet enrichTweet
...
export const TestimonialCard = ({
  ...
  const initial = clientName.trim().slice(0, 1).toUpperCase();
  ...
  return (
    <div
      ...
      {...props}
    >
      <div className="flex flex-row items-start tracking-normal">
        <div className="flex items-center space-x-3">
          {image?.trim() ? (
            <Image
              title={`Profile picture of ${clientName}`}
              alt={clientName}
              height={48}
              width={48}
              src={image}
              className="border-border/50 size-12 shrink-0 overflow-hidden rounded-full border object-cover"
            />
          ) : (
            <div
              className="border-border/50 flex size-12 shrink-0 items-center justify-center rounded-full border bg-muted text-sm font-medium text-muted-foreground"
              aria-hidden
            >
              {initial}
            </div>
      ...
      <p className="text-foreground text-[15px] leading-relaxed tracking-normal wrap-break-word">
        {content}
      </p>
    </div>
  );
